...
from ..stock_entry.test_stock_entry import create_test_parent_child_warehouses, create_test_item
...
class TestItem(FrappeTestCase):
	def test_sle_valuation_rate_on_item_creation(self):
		...
		item1 = create_test_item(
			"nord", "child_warehouse", 30, 4000
		)
		item_valuation_rate = frappe.db.get_value("Stock Ledger Entry", {"item": item1.name}, "valuation_rate")
