...
class StockEntry(Document):
	...
	def create_sle(self, warehouse,qty, stock_ledger_item):
		...
		item_stock_ledger_entries = stock_ledger_entries.select("*").where(item_field == item)

		ttl_valuation_rate = Sum(valuation_rate_field * qty_change_field).as_("val_rate_sum")
		response = item_stock_ledger_entries.select(ttl_valuation_rate).run(as_dict=True)
		...
		valuation_rate = (stock_ledger_old_valuation_rates_sum + (stock_ledger_item.rate * stock_ledger_item.qty)) / (stock_ledger_ttl_qtys + stock_ledger_item.qty)
